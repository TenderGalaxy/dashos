...
/**
 * Wrapper for running code that spans over multiple ticks via generator functions.
 * Async works via yield*, redirecting the "thread" into a function.
 * ```js
 * new Thread(function*(){
 *  console.log('Tick 1')
 *  yield
...
export class Thread {
  ...
  lazy() {
    return (res?: any) => {
      //@ts-expect-error
      ts.scheduleFirstUnused(() => this.step(res))
    }
  }
  step(res?: any) {
